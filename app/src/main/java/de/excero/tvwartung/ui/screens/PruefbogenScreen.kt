...
fun PruefbogenScreen(
    ...
    val vorschlag = buildString {
        ...
        punkte.forEach { p ->
            if (p.ergebnis == false && p.kurzbefundNiO.isNotBlank()) append(", ${p.kurzbefundNiO}")
        }
        ...
        Column(
            ...
            punkte.forEachIndexed { index, punkt ->
                PruefpunktCard(punkt) {
                    // Zusatzfelder je nach Prüfpunkt
                    when (index) {
            ...
            Button(
                onClick = {
                    ...
                    // Korrigierte Werte auf Wunsch in die Stammdaten übernehmen
                    var updated = current
                    if (serienUebernehmen && punkte[1].bemerkung.isNotBlank()) {
                        updated = updated.copy(seriennummer = punkte[1].bemerkung.trim())
                    }
                    if (freenetIdUebernehmen && punkte[2].bemerkung.isNotBlank()) {
                        updated = updated.copy(freenetId = punkte[2].bemerkung.trim())
                    }
                    if (updated != current) viewModel.updateRoom(updated)
