...
async function viewZeiterfassung() {
  ...
  const [dZeit, dBau, dMA] = await Promise.all([
    api(`/kkh/api/web/zeiterfassung?von=${wocheBeginn.toISOString().slice(0,10)}&bis=${heute.toISOString().slice(0,10)}&limit=500`),
    api('/kkh/api/web/baustellen'),
    api('/kkh/api/web/mitarbeiter'),
  ]);
  const eintraege  = dZeit.eintraege || [];
  const baustellen = dBau.baustellen || [];
  const mitarbeiter = (dMA.mitarbeiter || []).filter((m) => m.aktiv !== false);
  ...
  async function laden() {
    ...
    const d = await api(`/kkh/api/web/zeiterfassung?von=${von}&bis=${bis}&limit=500`);
  ...
  document.getElementById('ze-export').addEventListener('click', () => {
    const von = document.getElementById('ze-von')?.value || '';
    const bis = document.getElementById('ze-bis')?.value || '';
    window.open(`/kkh/api/web/export/zeiterfassung.xlsx?von=${von}&bis=${bis}`, '_blank');
  });
  document.getElementById('neu-ze-btn').addEventListener('click', () => editZeit(null, baustellen, mitarbeiter));
}

window.editZeit = async function(id, baustellenParam, maParam) {
  const baustellen = baustellenParam || (await api('/kkh/api/web/baustellen')).baustellen || [];
  const maListe    = maParam || ((await api('/kkh/api/web/mitarbeiter')).mitarbeiter || []).filter((m) => m.aktiv !== false);
  let alt = null;
  if (id) {
    const d = await api(`/kkh/api/web/zeiterfassung?limit=1000`);
    alt = (d.eintraege || []).find((e) => e.id === id);
  }

  const maOpts = maListe.map((m) => `<option value="${m.name}" ${alt?.mitarbeiter === m.name ? 'selected' : ''}>${m.name}</option>`).join('');
  ...
  const res = await modal(id ? 'Zeiteintrag bearbeiten' : 'Neuer Zeiteintrag', `
    <div class="form-grid">
      <div class="form-group"><label class="form-label">Mitarbeiter</label>
        <select class="form-control" id="ze-ma">${maOpts}</select></div>
  ...
  try {
    if (id) await api(`/kkh/api/web/zeiterfassung/${id}`, { method: 'PATCH', body });
    else await api('/kkh/api/web/zeiterfassung', { method: 'POST', body });
    toast('Gespeichert');
    viewZeiterfassung();
...
window.delZeit = async function(id) {
  ...
  await api(`/kkh/api/web/zeiterfassung/${id}`, { method: 'DELETE' });
  toast('Gelöscht');
  viewZeiterfassung();
};
