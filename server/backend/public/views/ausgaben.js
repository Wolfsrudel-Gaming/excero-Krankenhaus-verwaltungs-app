...
async function viewAusgaben() {
  ...
  const [dAusg, dBau] = await Promise.all([
    api(`/kkh/api/web/ausgaben?${params}`),
...
window.editAusgabe = async function(id, baustellenParam) {
  ...
  const alt = id ? (await api(`/kkh/api/web/ausgaben?` + new URLSearchParams({ firma_id: window.aktiveFirmaId || '' }))).ausgaben.find((a) => a.id === id) : null;
  ...
  try {
    if (id) await api(`/kkh/api/web/ausgaben/${id}`, { method: 'PATCH', body });
    else await api('/kkh/api/web/ausgaben', { method: 'POST', body });
    toast('Gespeichert');
    viewAusgaben();
...
window.delAusgabe = async function(id) {
  ...
  await api(`/kkh/api/web/ausgaben/${id}`, { method: 'DELETE' });
  toast('Gelöscht');
  viewAusgaben();
};
