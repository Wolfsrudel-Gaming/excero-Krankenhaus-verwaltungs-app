...
async function viewRechnungen() {
  ...
  const d = await api(`/kkh/api/web/rechnungen?${params}`);
  ...
  const grid = new DataGrid(document.getElementById('re-grid'), {
    columns: [
      ...
      { key: '_aktion', label: '', width: '120px',
        render: (_, row) => `
          <a href="/kkh/api/web/rechnungen/${row.id}/pdf" target="_blank" class="btn btn-ghost btn-xs">PDF</a>
...
window.editRechnung = async function(id) {
  const d = await api(`/kkh/api/web/rechnungen/${id}`);
  const re = d.rechnung;
  ...
  const res = await modal(`Rechnung ${re.nummer}`, `
    ...
    <div style="margin-top:8px;display:flex;gap:8px">
      <a href="/kkh/api/web/rechnungen/${id}/pdf" target="_blank" class="btn btn-secondary btn-sm">PDF öffnen</a>
  ...
  setTimeout(() => {
    ...
    document.getElementById('versenden-btn')?.addEventListener('click', async () => {
      ...
      if (!email) return;
      try {
        await api(`/kkh/api/web/rechnungen/${id}/versenden`, { method: 'POST', body: { an: email } });
    ...
    document.getElementById('storno-btn')?.addEventListener('click', async () => {
      ...
      await api(`/kkh/api/web/rechnungen/${id}/storno`, { method: 'POST' });
  ...
  try {
    await api(`/kkh/api/web/rechnungen/${id}`, { method: 'PATCH', body: {
    ...
    await api(`/kkh/api/web/rechnungen/${id}/positionen`, { method: 'PUT', body: { positionen } });
...
async function neueRechnung() {
  ...
  const nr = await api(`/kkh/api/web/rechnungen/next-nr?firma_id=${firmaId}`);
  ...
  if (res !== 'ok') return;
  try {
    const d = await api('/kkh/api/web/rechnungen', { method: 'POST', body: {
      firma_id: firmaId,
...
async function rechnungAusAbrechnung() {
  ...
  if (res !== 'ok') return;
  try {
    const d = await api('/kkh/api/web/rechnungen/aus-abrechnung', { method: 'POST', body: {
      firma_id: firmaId,
