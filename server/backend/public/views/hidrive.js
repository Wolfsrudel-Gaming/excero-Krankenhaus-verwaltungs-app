...
async function hdRender(pfad) {
  ...
  let d;
  try {
    d = await api(`/kkh/api/web/hidrive/list?pfad=${encodeURIComponent(pfad)}`);
  } catch (e) {
        ...
        ${eintraege.map((e) => `
          <tr>
            ...
            <td style="text-align:right">
              ${!e.isOrdner ? `<a href="/kkh/api/web/hidrive/download?pfad=${encodeURIComponent(e.href)}" class="btn btn-ghost btn-xs" download>⬇️</a>` : ''}
  ...
  cont.querySelectorAll('.hd-del').forEach((el) => {
    el.addEventListener('click', async () => {
      ...
      try {
        await api(`/kkh/api/web/hidrive/delete?pfad=${encodeURIComponent(el.dataset.pfad)}`, { method: 'DELETE' });
  ...
  if (uploadBtn && fileInput) {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      try {
        ...
        await fetch(`/kkh/api/web/hidrive/upload?pfad=${encodeURIComponent(zielPfad)}`, {
          method: 'PUT',
