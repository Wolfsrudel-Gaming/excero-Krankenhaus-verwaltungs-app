...
async function viewBaustellen() {
  ...
  const [dBau, dFirmen, dKunden] = await Promise.all([
    api(`/excero/api/web/baustellen${firmaId ? `?firma_id=${firmaId}` : ''}`),
    api('/excero/api/web/firmen'),
    api('/excero/api/web/kunden'),
  ]);
...
window.editBaustelle = async function(id, firmenParam, kundenParam) {
  const firmen = firmenParam || (await api('/excero/api/web/firmen')).firmen || [];
  const kunden = kundenParam || (await api('/excero/api/web/kunden')).kunden || [];
  let alt = null;
  if (id) {
    const d = await api(`/excero/api/web/baustellen`);
  ...
  try {
    if (id) await api(`/excero/api/web/baustellen/${id}`, { method: 'PATCH', body });
    else await api('/excero/api/web/baustellen', { method: 'POST', body });
    toast('Gespeichert');
    viewBaustellen();
